...
switch ($action) {
  ...
  case 'do_insertVehicle':
    ...
    if (empty($invMake) || empty($invModel) || empty($invDescription) || empty($invPrice) || empty($invMiles) || empty($invColor) || empty($classificationId)) {
      $message = "<p>Sorry! Please fill out all areas. Please try again.</p>";
      include '../views/add-vehicle.php';
      exit;
    } else {
      if (($imgMatch) || empty($imgName)) {
        $invImage = $noImage;
        $invThumbnail = "no-image-tn.png";
      };
